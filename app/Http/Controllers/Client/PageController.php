...
class PageController extends Controller
{
    ...
    public function aboutUs(){
        ...
        $data['pageContent'] = PageContent::where(['slug'=>'gioi-thieu'])->first(['id','title','content','description']);
    ...
    {
        $data['list'] = Services::where('status', 1)->select('name', 'description', 'slug', 'category_slug', 'type_slug')->paginate(9);
        $data['title'] = 'Tất cả dịch vụ';
        return view('service.list', $data); 
    }

    public function listServiceCate($cate)
    {
        $data['list'] = Services::where(['status'=>1 ,'category_slug'=>$cate])->select('name', 'description', 'slug', 'category_slug', 'type_slug')->paginate(6);
        $data['cate'] = ServiceCategory::where(['status'=>1 ,'slug'=>$cate])->first();
        $data['title'] = languageName($data['cate']->name);
        return view('service.list', $data);
    }

    public function listServiceType($cate, $type)
    {
        $data['list'] = Services::where([ 'status'=>1, 'category_slug'=>$cate, 'type_slug'=>$type])->select('name', 'description', 'slug', 'category_slug', 'type_slug')->paginate(6);
        $data['cate'] = ServiceCategory::where(['status'=>1 ,'slug'=> $cate])->first();
        $data['cate_title'] = languageName($data['cate']->name);
        $data['type'] = ServiceCategoryType::where(['status'=>1 ,'slug'=> $type])->first();
        $data['title'] = languageName($data['type']->name);
        return view('service.list', $data);
    }

    public function serviceDetail($cate, $type, $slug)
    {
        $data['detail_service'] = Services::where(['status'=>1, 'slug'=>$slug])->first();
        $data['cate'] = ServiceCategory::where(['status'=>1 ,'slug'=> $cate])->first();
        $data['cate_title'] = languageName($data['cate']->name);
        $data['type'] = ServiceCategoryType::where(['status'=>1 ,'slug'=> $type])->first();
        $data['type_title'] = languageName($data['type']->name);
